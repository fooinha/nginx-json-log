...
static char *        ngx_http_log_json_format_block(ngx_conf_t *cf,
                                                    ngx_command_t *cmd,
                                                    void *conf);
...
ngx_http_log_json_find_saved_parent(ngx_pool_t *pool,
        ngx_array_t *arr_levels, ngx_str_t *name, size_t len) {
...
static json_t *
ngx_http_log_json_find_parent(ngx_pool_t *pool, ngx_array_t *arr_levels,
    ...
    while((pos = ngx_http_log_json_has_label_pos(pos)) !=NULL) {
        ...
        ngx_http_log_json_value_t * saved =
            ngx_http_log_json_find_saved_parent(pool, arr_levels, path, len);
...
static void ngx_http_log_json_add_json_node(ngx_pool_t *pool,
        json_t *base, int is_array, json_type type, const char *key,
        ngx_str_t *value) {
...
static ngx_int_t ngx_http_log_json_log_handler(ngx_http_request_t *r) {
    ...
    /* Discard connect methods ... somehow file is not open. !? */
    ...
    /* get format */
    ngx_http_variable_value_t * format =
    ...
    /* If no mixed and prepared items for format */
    kv = (ngx_http_log_json_variable_t *) format->data;
    ...
    for (i = 0; i < kv->mixed->nelts; i++) {
        ...
        if (levels) {
            ...
            parent = ngx_http_log_json_find_parent(r->pool,
                    arr_levels, parent, cv[i].name);
        }
        ...
        const char *key = ngx_http_log_json_label_key_dup(r->pool,
                cv[i].name, cv[i].name->len);
        ngx_http_log_json_add_json_node(r->pool,
                parent, cv[i].is_array, cv[i].type, key, &value);

    } // mixed loop

    char *txt = json_dumps(obj,
    ...
    if (kv->sink_type == NGX_HTTP_LOG_JSON_SINK_FILE) {
        if (ngx_http_log_json_write_sink_file(klcf->file->fd,
                    txt) == NGX_ERROR) {
    ...
    if (kv->sink_type == NGX_HTTP_LOG_JSON_SINK_KAFKA){
        ...
        if (klcf->kafka.rkt == NGX_CONF_UNSET_PTR || !klcf->kafka.rkt)  {
            ...
            http_log_json_kafka_topic_conf_set_str(r->pool, klcf->kafka.rktc,
                    conf_req_required_acks_key, &conf_zero_value);

            /* configure and create topic */
            klcf->kafka.rkt = http_log_json_kafka_topic_new(r->pool,
                    mcf->kafka.rk, klcf->kafka.rktc, &klcf->kafka.topic);
            if (! klcf->kafka.rkt) {
...
static ngx_int_t
ngx_http_log_json_format_variable(ngx_http_request_t *r,
...
static ngx_int_t
ngx_http_log_json_items_init(ngx_http_log_json_variable_t *kv,
        ngx_pool_t *pool) {
    ...
    ngx_array_t * mixed = ngx_array_create(pool,
            kv->items_len, sizeof(ngx_http_log_json_item_t));
    if (!mixed) {
        return NGX_ERROR;
    }

    ngx_array_t * items = ngx_array_create(pool,
            kv->items_len, sizeof(ngx_str_t));
    if (!items) {
        return NGX_ERROR;
    }
...
/* Reads format from configuration */
static ngx_int_t
ngx_http_log_json_read_format(ngx_conf_t *cf,
        ngx_http_log_json_variable_t *kv, ngx_pool_t *pool) {
...
/* This requires PCRE */
    ...
    /* While we find group lines for the format */
    int matched = ngx_regex_exec(rc.regex, format, ovector, 1024);
    while (matched > 0) {
    int offset = 0;
        ...
        int i;
        for (i=0; i < matched; i++) {
            int ret = pcre_copy_substring((const char *)format->data,
        ...
        if (cv == NULL) {
            ngx_log_error(NGX_LOG_ERR, pool->log,
        ...
        matched = ngx_regex_exec(rc.regex, format, ovector, 1024);
    }
#endif

    /* sort items .... this is very import for serialization output alg*/
    ngx_sort(kv->mixed->elts, (size_t) kv->mixed->nelts,
...
ngx_http_log_json_format_block_parse_output_location(ngx_conf_t *cf,
        ngx_http_log_json_loc_conf_t* klcf,
        ngx_http_log_json_variable_t *kv, ngx_str_t *log) {

    if (! log) {
        goto failed;
    }

    /* check destination sink type */
    if (HTTP_LOG_JSON_LOG_HAS_FILE_PREFIX(log)) {
        ...
        ngx_cpystrn(klcf->filename.data,
                &log->data[HTTP_LOG_JSON_FILE_OUT_LEN], len);
    ...
    if (HTTP_LOG_JSON_LOG_HAS_KAFKA_PREFIX(log)) {
        ...
        ngx_cpystrn(klcf->kafka.topic.data,
                &log->data[HTTP_LOG_JSON_KAFKA_OUT_LEN], len);
    ...
    ngx_conf_log_error(NGX_LOG_NOTICE,
            cf, 0, "output location [%v]", log);
    return NGX_CONF_OK;
failed:
    ngx_conf_log_error(NGX_LOG_EMERG,
            cf, 0, "invalid format log output \"%v\"", log);
...
ngx_http_log_json_format_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (! value) {
        ngx_conf_log_error(NGX_LOG_EMERG,
                cf, 0, "invalid empty format", &value[1]);
        return NGX_CONF_ERROR;
    }

    v = ngx_http_add_variable(cf,
            &var_http_log_json_format, NGX_HTTP_VAR_CHANGEABLE);
    if (v == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    if (ngx_http_log_json_format_block_parse_output_location(cf,
                klcf, kv, &value[1]) == NGX_CONF_ERROR) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf,
                0, "invalid format log location");
        return NGX_CONF_ERROR;
    }

    kv->format = value[2];
    kv->items_len = ngx_http_log_json_count(&kv->format, ';');
    if (ngx_http_log_json_items_init(kv, cf->pool) != NGX_OK) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf,
                0, "invalid format init");
        return NGX_CONF_ERROR;
    }
